...
class Summarizer(object):
    ...
    def _score_sentences(self,sentences):
        ...
        for sentence in sentences:
            ...
            for value in self.freq_table:
                if value in sentence.lower():
                    word_not_in_stop_words += 1
                    self.sentence_value[sentence[:10]] += self.freq_table[value]
    ...
    def run(self):
        ...
        self.threshold = self._create_frequency_table()._score_sentences(sentence)._find_average_score()
        self.threshold *= 0.8
        summary = self._generate_summary(sentence)
        return summary
